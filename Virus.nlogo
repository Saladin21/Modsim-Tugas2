turtles-own
  [ sick?                ;; if true, the turtle is infectious
    ...
    vaccinated?]       ;;if true, the turte is vaccinated
globals
  [ %infected            ;; what % of the population is infectious
    ...
    ;;chance-vaccinate ;;chance vaccinated
...
to setup-turtles
  create-turtles number-people
    [ setxy random-xcor random-ycor
      ...
      get-healthy ]
  ask n-of (( Vaccinated / 100) * number-people ) turtles
    [ become-vaccinated ]
...
to get-healthy ;; turtle procedure
  set sick? false
  set vaccinated? false ;;not vaccinated
  set remaining-immunity 0
  set sick-time 0
end
...
to go
  ask turtles [
    get-older
    move
    ifelse sick? [ recover-or-die ] [ vaccinate ] ;;get vaccinated
    ifelse sick? [ infect ] [
      if can-reproduce[reproduce]
    ]
...
to update-display
  ask turtles
    [ if shape != turtle-shape [ set shape turtle-shape ]
      set color ifelse-value sick? [ red ] [ ifelse-value immune? [ grey ] [ifelse-value vaccinated? [ orange ] [green] ] ] ]
end
...
to get-older ;; turtle procedure
  ...
  set age age + 1
  if die-old and age > lifespan [ die ]
...
to infect ;; turtle procedure
  ask other turtles-here with [ not sick? and not immune? and not vaccinated?] ;;new bool vaccinated
...
;; Get vaccinated
;; Immune turtles don't get sick.
to vaccinate ;; turtle procedure
  if random-float 100 < chance-vaccinate[
    become-vaccinated
  ]
end
...
"healthy" 1.0 0 -10899396 true "" "plot count turtles with [ not sick? and not immune? and not vaccinated?]"
"total" 1.0 0 -13345367 true "" "plot count turtles"
"vaccinated" 1.0 0 -955883 true "" "plot count turtles with [ vaccinated? ]"
